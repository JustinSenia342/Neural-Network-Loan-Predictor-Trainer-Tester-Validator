...
public class NeuralNetworkLoan
{
	/***********************************************************************************/
	...
	private double[] preProcColMin = {500.0, 30.0, 30.0, 0.0, 0.0, 0.0};	//keeps track of column minimum values from training
	private double[] preProcColMax = {900.0, 90.0, 80.0, 1.0, 1.0, 1.0};	//keeps track of column maximum values from training
	...
	public void preProcessFile(Scanner sIn, PrintWriter pWrite, char typeFlag)
	{
		...
		//string to hold temporary read in values
		String tempStore = "";
		
		//reads in data from external file, 
		//converts all string values to numbers
		for (int i = 0; i < numDataClusters; i++)
		{
			for (int j = 0; j < numInputAndOutput; j++)
			{
				
				//multiple cases, each corresponding to a data column
				if (j == 0)
				{
					preProcArray[i][j] = (double) sIn.nextInt();
				}
				else if (j == 1)
				{
					preProcArray[i][j] = (double) sIn.nextInt();
				...
				{
					tempStore = sIn.next();
					if 		(tempStore.equalsIgnoreCase("male"))
					{
						preProcArray[i][j] = 1.0;
					}
					else if (tempStore.equalsIgnoreCase("female"))
					{
						preProcArray[i][j] = 0.0;
					}
				}
				else if (j == 4)
				{
					tempStore = sIn.next();
					if 		(tempStore.equalsIgnoreCase("single"))
					{
						preProcArray[i][j] = 0.0;
					}
					else if (tempStore.equalsIgnoreCase("divorced"))
					{
						preProcArray[i][j] = 0.5;
					}
					else if (tempStore.equalsIgnoreCase("married"))
					{
						preProcArray[i][j] = 1.0;
					}
				}
				else if (j == 5 && typeFlag != 'R')
				{
					tempStore = sIn.nextLine();
					tempStore = tempStore.trim();
					
					if 		(tempStore.equalsIgnoreCase("undetermined"))
					{
						preProcArray[i][j] = 1.0;
					}
					else if (tempStore.equalsIgnoreCase("high"))
					{
						preProcArray[i][j] = 0.66;
					}
					else if (tempStore.equalsIgnoreCase("medium"))
					{
						preProcArray[i][j] = 0.33;
					}
					else if (tempStore.equalsIgnoreCase("low"))
					{
						preProcArray[i][j] = 0.0;
					}
				}
			...
			for (int j = 0; j < numberOutputs; j++)
			{
				if (typeFlag == 'V') //if validation file
				{
					if (j == 0 || j == 1 || j == 2)
					{
						...
						{tempStr2 = "undetermined";}
						
						System.out.printf("(Actual Output = %14s Predicted Output = %14s)", tempStr1, tempStr2);
						pWrite.printf("(Actual Output = %14s Predicted Output = %14s)", tempStr1, tempStr2);
					}					
				}
				else if (typeFlag == 'R') //if run/test file
				{
					...
					System.out.printf("%14s ", tempStr1);
					pWrite.printf("%14s ", tempStr1);
				}
			}
			System.out.println("");
			pWrite.println("");
